...
def get_user_profile():
    ...
    try:
        ...
        try:
            profile_result = supabase_service.supabase.table('profiles').select('*').eq('id', user_id).single().execute()
            ...
            if not profile_result.data:
                ...
                try:
                    auth_result = supabase_service.supabase.auth.admin.get_user_by_id(user_id)
                    if auth_result.user:
                        default_profile['email'] = auth_result.user.email
                except:
                    pass
                
                # Insert default profile
                insert_result = supabase_service.supabase.table('profiles').insert(default_profile).execute()
            ...
            # Return a minimal profile if table doesn't exist
            return jsonify({
                'status': 'success',
                'profile': {
                    'id': user_id,
        ...
        return jsonify({
            ...
            'profile': {
                ...
                'has_health_condition': profile.get('has_health_condition', False),
...
def update_user_profile():
    ...
    try:
        ...
        # Health Information
        if 'has_health_condition' in data:
            update_data['has_health_condition'] = data['has_health_condition']
        ...
        result = supabase_service.supabase.table('profiles').update(update_data).eq('id', user_id).execute()
        ...
        return jsonify({
            ...
            'profile': {
                ...
                'display_name': f"{updated_profile.get('first_name', '')} {updated_profile.get('last_name', '')}".strip(),
...
def health_check():
    ...
    try:
        ...
        if supabase_service:
            ...
            result = supabase_service.supabase.table('profiles').select('id').limit(1).execute()
            db_status = 'healthy'
        else:
            db_status = 'no_service'
